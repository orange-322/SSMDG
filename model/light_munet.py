...
def get_dwconv_layer(spatial_dims: int,
                     in_channels: int,
                     out_channels: int,
                     kernel_size: int = 3,
                     stride: int = 1,
                     bias: bool = False):
    depth_conv = Convolution(spatial_dims=spatial_dims,
                             ...
                             conv_only=True,
                             groups=in_channels)
    point_conv = Convolution(spatial_dims=spatial_dims,
                             in_channels=in_channels,
...
def get_rvm_layer(spatial_dims: int,
                  in_channels: int,
                  out_channels: int,
                  stride: int = 1):
    ...
    if stride != 1:
        if spatial_dims == 2:
            return nn.Sequential(
                mamba_layer, nn.MaxPool2d(kernel_size=stride, stride=stride))
        if spatial_dims == 3:
            return nn.Sequential(
                mamba_layer, nn.MaxPool3d(kernel_size=stride, stride=stride))
    return mamba_layer
...
class ResMambaBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = get_rvm_layer(spatial_dims,
                                   in_channels=in_channels,
                                   out_channels=in_channels)
        self.conv2 = get_rvm_layer(spatial_dims,
                                   in_channels=in_channels,
                                   out_channels=in_channels)
...
class ResUpBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm2 = get_norm_layer(name=norm,
                                    ...
                                    channels=in_channels)
        self.act = get_act_layer(act)
        self.conv = get_dwconv_layer(spatial_dims,
                                     in_channels=in_channels,
                                     out_channels=in_channels,
                                     kernel_size=kernel_size)
        self.skip_scale = nn.Parameter(torch.ones(1))
...
class LightMUNet(nn.Module):
    ...
    def _make_down_layers(self):
        ...
        blocks_down, spatial_dims, filters, norm = (self.blocks_down,
                                                    self.spatial_dims,
                                                    self.init_filters,
                                                    self.norm)
        for i, item in enumerate(blocks_down):
            layer_in_channels = filters * 2**i
            downsample_mamba = (get_rvm_layer(spatial_dims,
                                              layer_in_channels // 2,
                                              layer_in_channels,
                                              stride=2)
            ...
            down_layer = nn.Sequential(
                downsample_mamba, *[
                    ResMambaBlock(spatial_dims,
                                  layer_in_channels,
                                  norm=norm,
                                  act=self.act) for _ in range(item)
                ])
            ...
            up_layers.append(
                nn.Sequential(*[
                    ResBlock(spatial_dims,
                             ...
                             norm=norm,
                             act=self.act) for _ in range(blocks_up[i])
                ]))
            up_samples.append(
                nn.Sequential(*[
    ...
    def decode(self, x: torch.Tensor,
               down_x: list[torch.Tensor]) -> torch.Tensor:
